...
class run:
            ...
            for e in range(1, self.epochs+1):
                ...
                try:
                    while True:
                        ...
                        step += 1
                        global_step += 1
                        print("estamos dentro del while y valor del estepp  ", step)
                        print("el global stepp es   ", global_step)
                ...
                try:
                    ...
                    print("divicion para cero nose bien")
                save_path = saver.save(sess, self.ckpt_path + "edsr_ckpt")
    ...
    def test(self, path):
        ...
        """
        image_list = sorted(glob.glob('{}/*'.format(path)))
        archivo3 = open('estadisticas/psnr.txt', 'w')
        salida='images/'
        contador=0
        ...
        for i, image_path in enumerate(image_list):
            ...
            with tf.Session(config=self.config) as sess:
                ...
                metrica = self.psnr(cropped, HR_image)
                valor=float(metrica)
                total=round(valor, 2)
                presentar=str(total)
                ...
                print("PSNR of  EDSR   upscaled image: "+  presentar)
                contador = contador+1
                contar=str(contador)
                archivo3.write(contar)
                #
                cadena = image_path[14:]
                print(cadena)
                ...
                #print("la ruta da la imagen es " +image_path)

                valor = image_path.split(os.sep)
                nombre = valor[1]
                #print (valor[1])

                cv2.imwrite(f'./salida/{nombre}', HR_image)
                #cv2.imwrite("./images/"+image_path, HR_image)
                #os.path.join(args.outputpath, cadena)
